...
folder = r'D:\file\git_code\program\hotfix_online\log'
...
def get_file_list():
    ...
    for file in os.listdir(folder):
        ...
        if os.path.isdir(d):
            for file_info in os.listdir(d):
                files.append(os.path.join(d, file_info))
    files.sort()
    return files
...
def boot():
    ...
    for fileName in files:
        # for uid in userId:
            print '===================', fileName,'==================='
            readFile(fileName, executeHeroInfoMsg, action)
