...
// 字母 'R' 和 'D'分别代表了 Radiant（天辉）和 Dire（夜魇）。然后，如果有 n 个参议员，
// 给定字符串的大小将是 n。
...
func predictPartyVictory(senate string) string {
	var r_list, d_list []int
	for i, s := range senate {
		if s == 'R' {
			r_list = append(r_list, i)
		} else {
			d_list = append(d_list, i)
		}
	}
	fmt.Println(r_list, d_list)
	for len(r_list) > 0 && len(d_list) > 0 {
		if r_list[0] < d_list[0] {
			r_list = append(r_list, len(senate)+r_list[0])
		} else {
			d_list = append(d_list, len(senate)+d_list[0])
		}
		r_list = r_list[1:]
		d_list = d_list[1:]
		fmt.Println("2-->", r_list, d_list)
	}
	if len(r_list) > 0 {
		return "Radiant"
	}
